...
assessmentButton.addEventListener(
  'click', // クリックイベント
  () => {  // 無名関数でアロー関数
    const userName = userNameInput.value; //　入力欄inputから値取得
    if (userName.length === 0){ // もし入力が空なら
      // 名前が空の時は処理を終了する
      return; //このリターンは強制終了のリターン
    }
    ...
    const result = assessment(userName);//診断結果作成
    paragraph.innerText = result;//pタグの内側のテキスト
